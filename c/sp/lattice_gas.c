    ...
    for (sweep = 0; sweep < numberOfSweeps; sweep++) {
      ...
      if(((1 + sweep) % MEASUREMENT_PERIOD) == 0) {
	      ...
	      if(numMeasure >= NUM_MEASUREMENTS) {
	        printf("Error: number of measurements is too large\n");
...
double myInit(void) {
  ...
  printf("# Number of steps (time): %d\n", numberOfSweeps);
  printf("# Number of samples: %d\n", numberOfSamples);
  printf("# Number of measurements: %d\n", NUM_MEASUREMENTS);
  printf("# Measurement period: %d\n", MEASUREMENT_PERIOD);
  if ((NUM_MEASUREMENTS * MEASUREMENT_PERIOD) != numberOfSweeps) {
    printf("Error: the number of steps must be a multiple of the number of measurements\n");
    exit(MY_EXIT_FAILURE);
  }
  fprintf(fDat, "# a = %Lu m = 2^64 I0 = %Lu\n", a, seed);
  fprintf(fDat, "# Number of steps (time): %d\n", numberOfSweeps);
  fprintf(fDat, "# Number of samples: %d\n", numberOfSamples);
  fprintf(fDat, "# Number of measurements: %d\n", NUM_MEASUREMENTS);
  fprintf(fDat, "# Measurement period: %d\n", MEASUREMENT_PERIOD);

  #ifdef MY_DEBUG
  printf("# Debug mode active: the esecution speed will be slower.\n");
  fprintf(fDat, "# Debug mode active: the esecution speed will be slower.\n");
  #endif

  if (sizeof(RANDOM_TYPE) < 8) {
    printf("Error: RANDOM_TYPE should be at least 8 bytes,"
	   " but it is  %d\n", sizeof(RANDOM_TYPE));
    exit(MY_EXIT_FAILURE);
  }
  if (DIM != 2) {
    printf("Error: this program has been verified,"
	   " only in D = %d\n", DIM);
    exit(MY_EXIT_FAILURE);
  }
  ...
  printf("# Enter the density value:\n");
  scanf("%lg", &rho);
  particleNumber = (long) (rho * (double)VOLUME);
  printf("# density = %lg, number of particles = %ld\n", rho, particleNumber);
  fprintf(fDat, "# density = %lg, number of particles = %ld\n", rho, particleNumber);

  if(rho >= 1.0) {
    printf("Error: density too high: %lg\n", rho);
    exit(MY_EXIT_FAILURE);
  }
...
void myEnd(double averageRho) {
  ...
  fprintf(fDat, "# Average density = %lg\n", averageRho);
...
void updateLattice(long thisN) {
  ...
  for (number = 0; number < particleNumber; number++) {
    ...
    if(particleOfSite[x][y] != thisParticle) {
      printf("Internal error: particella indecisa x %ld y %ld P %ld TP %ld\n",
    ...
    if (this_direction == 0) {
      ...
      printf("Internal error: bad neighbors\n");
      exit(MY_EXIT_FAILURE);
    }
...
double measure(long thisN) {
  ...
  for (i = 0; i < thisN; i++) {
    double d0 = truePositionOfParticle[i][0] - zeroPositionOfParticle[i][0];
    double d1 = truePositionOfParticle[i][1] - zeroPositionOfParticle[i][1];
    r2 += d0 * d0 + d1 * d1;
  }
